public enum CmdKind: String, CaseIterable, Equatable, Sendable {
    ...
    case centerFloatingWindow = "center-floating-window"
    case close
    ...
    for kind in CmdKind.allCases {
        switch kind {
            ...
            case .centerFloatingWindow:
                result[kind.rawValue] = SubCommandParser(CenterFloatingWindowCmdArgs.init)
            case .close:
            ...
            case .notifications:
                result[kind.rawValue] = SubCommandParser(parseNotificationsCmdArgs)
            case .reloadConfig:
